...
(function(){
  ...
  function launch(){
    ...
    const hue=(Math.random()*360)|0, n=60+((Math.random()*30)|0);
    for(let i=0;i<n;i++){
      const a=6.283*i/n, s=1.6+Math.random()*2.8;
      ps.push({x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:60+Math.random()*25,h:hue});
    }
  }
  function step(){
    if(!run) return;
    ...
    for(let i=ps.length-1;i>=0;i--){
      ...
      ctx.fillStyle=`hsla(${p.h},100%,60%,${a})`;
  ...
  btn.className='fx-toggle'; btn.type='button';
  btn.textContent='🎇'; btn.title='เปิด/ปิดพลุ (กด F ก็ได้)';
